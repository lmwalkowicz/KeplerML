...
for i in range (62000, 63000):
	if i%1==0:
		...
		ids.append(tabledata.field('TIME'))
		opens.close()
...
x=np.array(myarray)
x=x.T
y=myarray[62000:63000:1][::]
d=[float(y[i][52])/float(y[i][53])for i in range(len(numpdc))]
mini=min(y[::][6])
for i in range(len(numpdc)):
	if (float(y[i][56])!=0)&(float(y[i][16])<0.8):
			print i
			print y[i][30],y[i][16], y[i][56],y[i][25],y[i][8] 
