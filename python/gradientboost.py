# runs the gradient boosting classifier on the training sets multiple times in various ways
...
for i in range(1,2):
    if i==1 or i==3:
        ...
        X_test = sampletest
        y_test = labels[172:,i]
    #overall best case: 60, 3, 2, 1
    posterior = np.empty([1,72,6])
    for j in range(60,61):
        for k in range(3,4):
            box = np.zeros([6,6])
            accuracy = np.zeros(72)
            for m in range(0,1):
                gbc = GradientBoostingClassifier(n_estimators=j, max_depth=k, min_samples_split=2, min_samples_leaf=1)
            ...
            if np.mean(accuracy)/0.72>=54.0: print j, k, np.mean(accuracy)/0.72
            print '{:3d} {:2d} {:2.4f} {:2.4f} {:2.4f} {:2.4f} {:2.4f} {:2.4f} {:2.4f}'.format(j,k,sum(accuracy[0:8])/8.0, sum(accuracy[8:18])/10.0, sum(accuracy[18:30])/12.0, sum(accuracy[56:72])/16.0, sum(accuracy[30:43])/13.0, sum(accuracy[43:56])/13.0, sum(accuracy)/72.0)
        '''
    means = np.empty([72,6])
    stds = np.empty([72,6])
    grid = np.empty([6,6])
