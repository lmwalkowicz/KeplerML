# tests the Stochastic Gradient Descent classfier over many runs
# to find the best value of n_components
# scratch that; it cut cuts the list into 6 parts in a way that gives the most accurate results
...
for i in range(1,2):
    ...
    for j in range(100,101):
        ...
        for m in range(0,100):
            sgd = linear_model.SGDClassifier(n_iter=j, shuffle=True)
            ...
            print m, y_pred, y_test
            #posterior[m] =  sgd.predict_proba(X_test)
        print j, np.mean(n)/0.72, np.std(n)/0.72
