# runs the extremely randomized trees classifier on the training sets multiple times in various ways
...
for i in range(1,2):
    if i==1 or i==3:
        ...
        X_test = sampletest
        y_test = labels[172:,i]
    # overall highest accuracy parameters: 70, 5, 2, 5
    ...
    for j in range(1,2):
        for k in range(1,2):
            ...
            for m in range(0,100):
                etc = ExtraTreesClassifier(n_estimators=70, max_depth=5, min_samples_split=2, min_samples_leaf=5)
                ...
                posterior[m] =  etc.predict_proba(X_test)
            #if np.mean(accuracy)/0.72>=57.0: print j, k, np.mean(accuracy)/0.72, np.std(accuracy)/0.72
            print '{:2d} {:2d} {:2.2f} {:2.2f} {:2.2f} {:2.2f} {:2.2f} {:2.2f} {:2.2f}'.format(j, k, sum(accuracy[0:8])/8.0, sum(accuracy[8:18])/10.0, sum(accuracy[18:30])/12.0, sum(accuracy[56:72])/16.0, sum(accuracy[30:43])/13.0, sum(accuracy[43:56])/13.0, sum(accuracy)/72.0)
