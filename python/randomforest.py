# runs the random forest classifier on the training sets multiple times in various ways
# to better characterize its function and results
...
for i in range(1,2):
    if i==1 or i==3:
        ...
        X_test = sampletest
        y_test = labels[172:,i]
        #overall best accuracy parameters: 30, 20
    for j in range(30,31):
        for k in range(20,21):
            ...
            for m in range(0,100):
                ...
                posterior[m] =  rfc.predict_proba(X_test)
            print '{:3d} {:2d} {:3.2f} {:2.2f} {:2.2f} {:2.2f} {:2.2f} {:2.2f} {:2.2f}'.format(j, k, sum(accuracy[0:8])/8.0, sum(accuracy[8:18])/10.0, sum(accuracy[18:30])/12.0, sum(accuracy[30:43])/13.0, sum(accuracy[43:56])/13.0, sum(accuracy[56:72])/16.0, sum(accuracy)/72.0)
    means = np.empty([72,6])
    stds = np.empty([72,6])
    grid = np.empty([6,6])
    for i in range(0,72):
    ...
    for j in range(0,6):
        ...
        #grid[1,j] = np.std(posterior[:,0:8,j])
        grid[1,j] = np.mean(posterior[:,8:18,j])
        #grid[3,j] = np.std(posterior[:,8:18,j])
        grid[2,j] = np.mean(posterior[:,18:30,j])
        #grid[5,j] = np.std(posterior[:,18:30,j])
        grid[3,j] = np.mean(posterior[:,30:43,j])
        #grid[7,j] = np.std(posterior[:,30:43,j])
        grid[4,j] = np.mean(posterior[:,43:56,j])
        #grid[9,j] = np.std(posterior[:,43:56,j])
        grid[5,j] = np.mean(posterior[:,56:72,j])
        #grid[11,j] = np.std(posterior[:,56:72,j])
