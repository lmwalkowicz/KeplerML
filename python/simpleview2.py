...
"""
plt.scatter(numtime[233],numpdc[233])
plt.show()
plt.scatter(numtime[32],numpdc[32])
plt.show()
sys.exit()
"""
"""
...
y=myarray[0:161145:1][::]

#gasymm>1.5(rr lyraes), normamp>1 or coeffvar>0.3, roundandzc>1000, posoutliers>40(75 is good enough), 35>100000,37>100, roundrat>500
#d=[float(y[i][52])/float(y[i][53])for i in range(len(numpdc))]
mini=min(y[::][6])
for i in range(len(y)):
	if (float(y[i][30])>=2500)&(float(y[i][56])<=0):
			print i
			print y[i][30],y[i][43], y[i][16], y[i][56],y[i][25],y[i][8], y[i][58] , myarray2[i][0]
			#print i,' ',y[i][40], y[i][6], y[i][53],y[i][56] 
			numpdc=[]
			numtime=[]

			opens=pyfits.open(myarray2[i][0])
		        table=opens[1]
			header=table.header
			tabledata=table.data
			test=tabledata.field('PDCSAP_FLUX').copy()
			time=tabledata.field('TIME').copy()
			#print header
			#name=tabledata.field('KEPLERID').copy()
			numpdc.append([test[value] for value in range(len(test)) if not ((math.isnan(test[value])) or (math.isnan(time[value])))])
			numtime.append([time[value] for value in range(len(test)) if not ((math.isnan(test[value])) or (math.isnan(time[value])))])
			
			plt.scatter(numtime,numpdc,color='red')
			plt.show()
...
time.append(tabledata.field('TIME').copy())
err.append(tabledata.field('PDCSAP_FLUX_ERR').copy())
#ids.append(tabledata.field('TIME').copy())
opens.close()
...
teff=[]
ids=[]
for i in range (84000, 114000):
	if i%1==0:
		opens=pyfits.open(myarray[i][0])
		table=opens[1]
		header=table.header
		tabledata=table.data
		test.append(tabledata.field('PDCSAP_FLUX').copy())
		time.append(tabledata.field('TIME').copy())
		err.append(tabledata.field('PDCSAP_FLUX_ERR').copy())
		#ids.append(tabledata.field('TIME').copy())
		opens.close()
