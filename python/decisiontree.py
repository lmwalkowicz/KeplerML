# runs the decision tree classifier on the training sets multiple times in various ways
    ...
    for j in range(1,2):
        for k in range(1,2):
            ...
            for m in range(0,10000):
                dtc = DecisionTreeClassifier(max_features=11, max_depth=10, min_samples_split=7, min_samples_leaf=8)
            ...
            if np.mean(accuracy)/0.72>=48.0: print j, k, np.mean(accuracy)/0.72, np.std(accuracy)/0.72
